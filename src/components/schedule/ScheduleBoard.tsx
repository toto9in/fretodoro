...
export function ScheduleBoard() {
  ...
  const handleDragStart = (event: DragStartEvent) => {
    ...
    if (!day) return;
    const block = liveRoutines.find(r => r.day_of_week === day)?.blocks.find(b => b.id === blockId);
  ...
  const handleDragOver = (event: DragOverEvent) => {
    ...
    if (fromDay === toDay) {
      ...
      const block = liveRoutines.find(r => r.day_of_week === fromDay)?.blocks.find(b => b.id === active.id);
      if (!block) return;

      const toRoutine = liveRoutines.find(r => r.day_of_week === toDay);
      const overIdx = toRoutine?.blocks.findIndex(b => b.id === over.id) ?? -1;
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (fromDay === toDay) {
      ...
      const toRoutine = liveRoutines.find(r => r.day_of_week === toDay);
      const toIndex = toRoutine?.blocks.findIndex(b => b.id === active.id) ?? 0;
