...
export const AddBlockModal = React.forwardRef<AddBlockModalHandle, { day: DayOfWeek }>(
  function AddBlockModal({ day }, ref) {
    ...
    const handleSubmit = useCallback(async (e: React.FormEvent) => {
      ...
      if (!title.trim()) { setError('Informe o nome do exercício.'); return; }
      if (!duration || isNaN(mins) || mins <= 0) { setError('Informe uma duração válida em minutos.'); return; }
        ...
        <div className="modal-box">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            <fieldset className="fieldset">
              <legend className="fieldset-legend">{t('dayCard.newActivity')}</legend>
              <input
                type="text"
            ...
            <fieldset className="fieldset">
              <legend className="fieldset-legend">{t('dayCard.estimatedTime')} ({t('dayCard.min')})</legend>
              <input
                type="number"
                className="input input-bordered w-full"
                placeholder="Ex: 15"
            ...
            <div className="modal-action mt-2">
              ...
              <button type="submit" className="btn btn-primary">{t('dayCard.newActivity')}</button>
            </div>
          </form>
        </div>
